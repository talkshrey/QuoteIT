...
const sendWelcomeEmail = (email, name, role) => transporter.sendMail({
  ...
  text: `Thanks for joining us, ${name}. QuoteIT is the best place for the best prices! Get started now by ${role === 'client' ? 'raising a query' : 'posting a quotation' }! Regards, QuoteIT`
}, (err, info) => {
  if(err){
